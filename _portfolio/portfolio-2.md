...
Recently, we investigated the Babu&scaron;ka or plate paradox, where convergence fails when a domain with curved boundary is approximated by polygonal domains in linear bending problems with simple support boundary conditions. We showed that the paradox also occurs for a nonlinear bending-folding model which enforces vanishing Gaussian curvature, and it can be cured by cutting the interface edges [[2026](https://doi.org/10.4171/IFB/566)]. 
